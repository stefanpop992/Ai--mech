...
def upgrade() -> None:
    ...
    op.drop_constraint("fk_cars_user_id", "cars", type_="foreignkey", if_exists=True)
    op.drop_column("cars", "user_id", if_exists=True)


def downgrade() -> None:
    op.add_column("cars", sa.Column("user_id", sa.Integer(), nullable=True))
    op.create_foreign_key(
        "fk_cars_user_id", "cars", "users", ["user_id"], ["id"], ondelete="CASCADE"
    )
